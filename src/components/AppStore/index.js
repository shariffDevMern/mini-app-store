const tabsList = [
  ...
  class AppStore extends Component {
    ...
    getFilteredApps = () => {
      const {activeTab, searchVal} = this.state
      const activeTabAppsList = appsList.filter(
        eachApp => eachApp.category === activeTab,
      )
      return activeTabAppsList.filter(eachApp =>
    ...
    render() {
      const {activeTab, searchVal} = this.state
      const filteredTabAppsList = this.getFilteredApps()
  
      return (
        <div className="app-store-bg-container">
          <div className="header-container">
            <h1 className="app-store-text">App Store</h1>
            ...
            <ul className="tab-container">
              {tabsList.map(eachTab => (
                <TabItem
                  ...
                  selectedTab={eachTab.tabId === activeTab}
                  eachTab={eachTab}
                />
              ))}
            </ul>
          ...
          <div className="apps-container">
            <ul className="apps-section">
              {filteredTabAppsList.length > 0 ? (
                filteredTabAppsList.map(eachApp => (
